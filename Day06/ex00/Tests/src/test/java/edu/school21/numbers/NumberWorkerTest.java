...
public class NumberWorkerTest {
    ...
    void isPrimeForIncorrectNumbers(int invalidNumber) {
        Assertions.assertThrows(IllegalNumberException.class, ()->numberWorker.isPrime(invalidNumber));
    }

    @ParameterizedTest
    @CsvFileSource(resources = "/data.csv")
    void testDigitsSum(int number, int expectedNumber){
        Assertions.assertEquals(numberWorker.digitsSum(number), expectedNumber);
    }
}
